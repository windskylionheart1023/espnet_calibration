...
class BatchHypothesis(NamedTuple):
    ...
    scores_list_before_pre_beam: List[Any] = []
...
class BatchBeamSearch(BeamSearch):
    ...
    def batchfy(self, hyps: List[Hypothesis]) -> BatchHypothesis:
        ...
        return BatchHypothesis(
            ...
            scores_list_before_pre_beam = [h.scores_list_before_pre_beam for h in hyps],
        )
    ...
    def _batch_select(self, hyps: BatchHypothesis, ids: List[int]) -> BatchHypothesis:
        ...
        return BatchHypothesis(
            ...
            hs=hs,
            scores_list=[hyps.scores_list[i] for i in ids.tolist()],
            scores_list_before_pre_beam=[hyps.scores_list_before_pre_beam[i] for i in ids.tolist()],
        )
    ...
    def _select(self, hyps: BatchHypothesis, i: int) -> Hypothesis:
        return Hypothesis(
            ...
            scores_list_before_pre_beam=hyps.scores_list_before_pre_beam[i],
        )
    ...
    def unbatchfy(self, batch_hyps: BatchHypothesis) -> List[Hypothesis]:
        ...
        return [
            Hypothesis(
                ...
                scores_list=batch_hyps.scores_list[i],
                scores_list_before_pre_beam=batch_hyps.scores_list_before_pre_beam[i],
            )
    ...
    def search(
        ...
        weighted_scores_before_pre_beam = weighted_scores.clone()
        ...
        weighted_scores_before_pre_beam += running_hyps.score.to(
            dtype=x.dtype, device=x.device
        ).unsqueeze(1)
        # temperature = torch.tensor([1.2550], device=scores[list(scores.keys())[0]].device, requires_grad=False)
        # if (ts:=False):
        ...
        for (
            ...
            sparse_dict = {str(index.item()): value.item() for index, value in zip(topk_output.indices, torch.softmax(topk_output[0], dim=0))}
            prob_list_before_pre_beam = torch.softmax(weighted_scores_before_pre_beam[full_prev_hyp_id], dim=0).tolist()
            sparse_dict_before_pre_beam = {index: value for index, value in enumerate(prob_list_before_pre_beam) if value != 0.0}
            ...
            hypo = Hypothesis(
                    ...
                    hs=new_hs,
                    scores_list=prev_hyp.scores_list + [sparse_dict],
                    scores_list_before_pre_beam=prev_hyp.scores_list_before_pre_beam + [sparse_dict_before_pre_beam],
                )
            best_hyps.append(hypo)
