...
class Speech2Text:
    ...
    def _decode_single_sample(self, enc: torch.Tensor) -> ListOfHypothesis:
        ...
        for hyp in nbest_hyps:
            ...
            # remove last logits which corresponding to eos
            hyp.scores_list.pop()
            hyp.scores_list_before_pre_beam.pop()
...
def inference(
    ...
    # import os
    # score_file_path = output_dir + '/1best_recog/scores_list.pth'
    # if (record_scores := True):
        # # Load the existing data
        # try:
        #     scores_list = torch.load(score_file_path)
        # except FileNotFoundError:
        #     scores_list = dict()
    # os.remove(score_file_path) if os.path.exists(score_file_path) else None
    scores_list = {key: [] for key in range(1, nbest + 1)}
    scores_list_before_pre_beam = {key: [] for key in range(1, nbest + 1)}
        ...
        for keys, batch in loader:
            ...
            if enh_s2t_task or multi_asr:
                ...
                for n, (text, token, token_int, hyp) in zip(
                    ...
                    # Add /update a new pair
                    scores_list[n].append((key, hyp.scores_list))
                    scores_list_before_pre_beam[n].append(
                        (key, hyp.scores_list_before_pre_beam)
                    )
    ...
    if (record_scores := True):
        for n in range(1, nbest+1):
            ...
            scores_before_pre_beam_file_path = f"{output_dir}/{n}best_recog/scores_list_before_pre_beam.json"
            with open(score_file_path, "w") as file:
                json.dump(scores_list[n], file)
            with open(scores_before_pre_beam_file_path, "w") as file:
                json.dump(scores_list_before_pre_beam[n], file)
...
def get_parser():
    ...
    group = parser.add_argument_group("Calibration related")
    group.add_argument(
        "--TS",
        type=str2bool,
        default=False,
        help="The trigger for temperature scaling",
    )
    group.add_argument(
        "--TS_factor",
        type=float,
        default=1.0,
        help="The factor for temperature scaling",
    )
    return parser
