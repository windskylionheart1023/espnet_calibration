import espnet2.asr.logsumexp_safe
import torch
...
def get_bandwidth(f, device):
    ...
    for b in bandwidths:
        ...
        log_fhat = torch.logsumexp(log_kern, dim=1, safe_inf=True) - torch.log(torch.tensor(n-1))
...
def get_ratio_canonical_log(f, y, bandwidth, p, device='cpu'):   
    ...
    log_den = torch.logsumexp(log_kern, dim=1, safe_inf=True)
    final_ratio = 0
    for k in range(f.shape[1]):
        if torch.sum(y_onehot, dim=0)[k].item() != 0:
            log_kern_y = log_kern + (torch.ones([f.shape[0], 1], device=device) * log_y[:, k].unsqueeze(0))
            log_inner_ratio = torch.logsumexp(log_kern_y, dim=1, safe_inf=True) - log_den
