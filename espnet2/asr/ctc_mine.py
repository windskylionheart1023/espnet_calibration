...
class CTC(torch.nn.Module):
    ...
    def forward(self, hs_pad, hlens, ys_pad, ys_lens):
        ...
        # # calibration branch
        # calibration = 1
        # from espnet2.asr.ece_kde import get_ece_kde
        # import torchaudio

        # if calibration :
        #     self.cali_lambda = 0.5
        #     ys_hat = ys_hat.softmax(2)
        #     f = ys_hat[:,0,:].unsqueeze(0) # porb socres for first utt
        #     y = ys_true[:207].unsqueeze(0) # labels for first utt

        #     alignment = torchaudio.functional.forced_align(torch.log(f), y)
        #     token_spans = torchaudio.functional.merge_tokens(alignment[0].squeeze(0), alignment[1].squeeze(0))
            
        #     loss_ece = get_ece_kde(f, y, bandwidth=0.001, p=1, mc_type='canonical', device='cpu')
        #     loss = loss + self.cali_lambda * loss_ece
