...
class LogSumExp_safe_inf(torch.autograd.Function):
    ...
    def forward(ctx, input, dim, keepdim):            
        with torch.enable_grad():
            output = torch.logsumexp_old(input, dim=dim, keepdim=keepdim) # internal copy of output
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, = torch.autograd.grad(output, input, grad_output, only_inputs=True)
...
def logaddexp_new(input: torch.Tensor, other: torch.Tensor, safe_inf: bool = False) -> torch.Tensor:
    ...
    if safe_inf:
        return LogAddExp_safe_inf.apply(input, other)
    else:
        return torch.logaddexp_old(input, other)
...
def logsumexp_new(x: torch.Tensor, *,
    ...
    in the backward pass when the result is ±∞."""
    if safe_inf:
        return LogSumExp_safe_inf.apply(x, dim, keepdim)
    else:
        return torch.logsumexp_old(x, dim=dim, keepdim=keepdim)
