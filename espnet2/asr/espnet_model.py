...
from espnet.nets.pytorch_backend.transformer.label_smoothing_loss import (  # noqa: H301
    LabelSmoothingLoss, FocalLabelSmoothingLoss, AlphaBetaDivLoss,
)
...
class ESPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        lang_token_id: int = -1,
        partly_calibration: bool = False,
        mmce: bool = False,
        kde: bool = False,
        focal: bool = False,
        base_attention_loss: str = "LS",
        ABdiv_conf: Optional[dict] = None,
        focal_gamma = 2.0,
        cali_lambda: int = 100,
    ):
        ...
        if self.use_transducer_decoder:
            ...
            # Define the base loss funtion in attention branch
            if base_attention_loss == "LS":
            ...
            elif base_attention_loss == "FocalLS":
                self.criterion_att = FocalLabelSmoothingLoss(
                    size=vocab_size,
                    padding_idx=ignore_id,
                    smoothing=lsm_weight,
                    normalize_length=length_normalized_loss,
            ...
            elif base_attention_loss == "ABdiv":
                self.criterion_att = FocalLabelSmoothingLoss(
                    size=vocab_size,
                    padding_idx=ignore_id,
                    smoothing=lsm_weight,
                    normalize_length=length_normalized_loss,
                    criterion=AlphaBetaDivLoss(alpha=ABdiv_conf["alpha"], beta=ABdiv_conf["beta"], reduction="none"),
                )
            ...
            #             normalize_length=length_normalized_loss,
            #             gamma=focal_gamma,
            #             criterion=AlphaBetaDivLoss(alpha=0.5, beta=0.5, reduction="none"),
            #         )
            #     else:
            #         self.criterion_att = FocalLabelSmoothingLoss(
            #             size=vocab_size,
        ...
        self.partly_calibration = partly_calibration
        self.mmce = mmce
        self.kde = kde
        self.cali_lambda = cali_lambda
        ...
        if self.use_transducer_decoder:
            ...
            if self.ctc_weight != 1.0:
                loss_att, acc_att, cer_att, wer_att = self._calc_att_loss(
                    ...
                    text,
                    text_lengths,
                    partly_calibration=self.partly_calibration,
                    mmce=self.mmce,
                    kde=self.kde,
                    cali_lambda=self.cali_lambda
                )
    ...
    def _calc_att_loss(
        ...
        if (calibration:=1):
            ...
            padding_idx = -1
            ignore = ys_out_pad == padding_idx
            # total = len(ys_out_pad) - ignore.sum().item()
            target = ys_out_pad.masked_fill(ignore, 0)  # avoid -1 index

            decoder_out_prob = decoder_out.softmax(2)
            # ave over batch
            # Example batch size, classes, and samples
            batch_size = decoder_out_prob.shape[0]

            # Iterate through the batch
            f_list = []
            y_list = []
            for i in range(batch_size):
                f_list.append(decoder_out_prob[i, :, :])  # Probabilities for the i-th batch element
                y_list.append(target[i, :])               # Target labels for the i-th batch element
            f = torch.cat(f_list, dim=0)
            y = torch.cat(y_list, dim=0)

            if partly_calibration:    
                mask = f < 1/f.shape[1]
                f = f * mask.float()
            ...
            if kde:
                ...
                # Compute ECE for this specific batch element
                bandwidth =  get_bandwidth(f, loss_att.device)
                loss_ece = get_ece_kde(f, y, bandwidth=bandwidth, p=1, mc_type='canonical', device=loss_att.device)
                
                # Optionally, you can average the loss over the batch
                loss_cali = loss_ece

            if mmce:
                if (unbiasd:=True):
                    from espnet2.asr.mmce import calibration_unbiased_loss
                    loss_mmce = calibration_unbiased_loss(f, y)
                else:
                    from espnet2.asr.mmce import calibration_mmce_w_loss
                    loss_mmce = calibration_mmce_w_loss(f, y)
                loss_cali = loss_mmce

            # combine ece loss with att loss
            loss_att = loss_att + cali_lambda * loss_cali
